...
TFile f_hist("../histogram/efficiency/after/test_2.root", "recreate");
...
void three_leaves()
{
    ...
    Long64_t max_event_testing = 24e6;
    start = std::clock();
    ...
    for(j=0;j<4;j++){
        for(i=0;i<16;i++){
            ...
            hPixieADC_ab[j][i]->Add(time_background[j][i],-(11.0/600.0));
            hPixieADC_eff[j][i]->Add(time_background_eff[j][i],-(15.0/600.0));
...
void second_loop(){
    ...
    //Double_t time_gate_lower=0;
    //Double_t time_gate_upper=1000;
    //ring_sector_time_gate=true;
    //ring_time=gamma_time;
    Int_t test_counter_g_cl=0;
    Int_t test_counter_g_ge=0;
    ...
    for (entry_id = 0, entry_size = TRenergy.GetSize(); entry_id < entry_size; ++entry_id) {
        ...
        if( mod==2&&chn==0 ){
            ...
            test_counter_g_ge++;
            if (test_counter_g_ge==1 && test_counter_p==1){
                time_Ge_test->Fill(time+200);
            }
            if (test_counter_g_ge==2 && test_counter_p==1){
                time_Ge_test2->Fill(time+200);
            }
        }
        ...
        if( (mod==1&&(chn==0||chn==1||chn==2||chn==3)) ){
            ...
            test_counter_g_cl++;
            if (test_counter_g_cl==1 && test_counter_p==1){
                time_Cl_test->Fill(time+200);
            }
            if (test_counter_g_cl==2 && test_counter_p==1){
                time_Cl_test2->Fill(time+200);
            }
        ...
        if(allready_filled==false&&calib_en>20&&calib_en<15000){
 ...
 */
 ...
 Double_t do_doppler(Double_t en, Int_t gamma_mod, Int_t gamma_chn){
     ...
     /*if (gamma_mod==3&&gamma_chn==5){
         cout << "cos alpha " << cos_alpha << " energy before " << en << " doppler shifted energy " << dop_cor_en << " theta_p " << theta[ring_mod][ring_chn]  << " phi_p " << phi[sector_mod][sector_chn] << " phi_gamma " << phi[gamma_mod][gamma_chn]<< " theta_gamma " << theta[gamma_mod][gamma_chn] << " cos(alpha) " << cos_alpha << endl;
        // cout << " " <<endl;
...
void init_histos() {
    ...
    time_Ge_test = new TH1F("time_labr_pg","",1500,0,1500);
    time_Ge_test2 = new TH1F("time_labr_pgg","",1500,0,1500);
    time_Cl_test = new TH1F("time_Cl1_pg","",1500,0,1500);
    time_Cl_test2 = new TH1F("time_Cl1_pgg","",1500,0,1500);
