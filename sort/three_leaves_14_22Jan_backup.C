...
TFile f_hist("../histogram/14C/01_18_2018_8.root", "recreate");
...
void second_loop(){
    ...
    for (entry_id = 0, entry_size = TRenergy.GetSize(); entry_id < entry_size; ++entry_id) {
        ...
        if (ab_event_cl1==true && chn==ab_use_crystal_cl1 && mod==1 && ring_sector_time_gate==true){
            ...
            if (abs(gamma_time-ring_time)<17 && (time+200)>time_gate_lower && (time+200)<time_gate_upper&&calib_en<10000){
                hPixieADC_pg[mod][chn]->Fill(fill_energy);
                hPixieADC_ab[mod][chn]->Fill(ab_use_energy_cl1);
            }
            if (abs(gamma_time-ring_time)<17 && (time+200)>background_gate_lower && (time+200)<background_gate_upper&&calib_en<10000){
