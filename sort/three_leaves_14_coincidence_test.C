...
TFile f_hist("../histogram/14C/04_10_2018_66.root", "recreate");
...
TH1F* coincidence_pgg_GE;
TH1F* coincidence_gg_GE;
TH1F* coincidence_pgg_ab_GE;
TH1F* coincidence_gg_ab_GE;
TH1F* coincidence_pgg_labr;
TH1F* coincidence_gg_labr;
TH1F* coincidence_pgg_ab_labr;
TH1F* coincidence_gg_ab_labr;
...
Bool_t coincidence_gate_GE = false;
Bool_t coincidence_gate_labr = false;
Double_t gamma_time2=0;
...
void three_leaves_14_coincidence_test()
{
    ...
    Long64_t max_event_testing = 6e9;
    start = std::clock();
    ...
    while (myReader.Next()) {
        ...
        if (coincidence_counter<2){
            coincidence_gate_GE=false;
            coincidence_gate_labr=false;
        }
        if (coincidence_gate_GE==true||coincidence_gate_labr==true){
            bad_gamma=false;
        }
...
void first_loop(){
    ...
    for (entry_id = 0, entry_size = TRenergy.GetSize(); entry_id < entry_size; ++entry_id) {
        ...
        if( ((mod==1 && (chn==0 || chn==1 || chn==2 || chn==3)) || (mod==3 && (chn==0 || chn==1 || chn==2 || chn==3||chn==5)) || (mod==2 && (chn==0 || chn==1 || chn==2 || chn==3 || chn==4 || chn==5)))){
            if(mod==2&&(chn==0 || chn==1 || chn==2 || chn==3 || chn==4 || chn==5)){
                temp_labr_en = do_calibrate(en,mod,chn);
            }
           // if ((calib_en>lower_coinc&&calib_en<upper_coinc)||(temp_labr_en>lower_coinc&&temp_labr_en<upper_coinc)){
            if ((calib_en>lower_coinc && calib_en<upper_coinc)){
                coincidence_gate_GE=true;
            }
            if ((temp_labr_en>lower_coinc && temp_labr_en<upper_coinc)){
                coincidence_gate_labr=true;
            }
            coincidence_counter++;
        }
...
void second_loop(){
    ...
    Double_t Labr_ds_coinc = 0;
    Double_t Labr_ab_coinc = 0;
    Double_t fill_energy_coinc=0;
    ...
    for (entry_id = 0, entry_size = TRenergy.GetSize(); entry_id < entry_size; ++entry_id) {
        ...
        if (mod==3&&chn==13){
            ...
            if((time+200)>time_gate_lower && (time+200)<time_gate_upper && abs(ring_en_for_hist - sector_en_for_hist)<diagonal_cut_width){
                one_ring_timegated->Fill(calib_en);
                if (abs(time-ring_time)<15){
        ...
        if (ab_event_cl1==true && chn==ab_use_crystal_cl1 && mod==1 && ring_sector_time_gate==true && abs(ring_en_for_hist - sector_en_for_hist)<diagonal_cut_width){
            ...
            if (abs(time-ring_time)<40 && (add_back_use_time1+200)>time_gate_lower && (add_back_use_time1+200)<time_gate_upper&&calib_en<10000){
                ...
                if(coincidence_gate_GE==true){
                    beta=do_beta(beta_ring_en);
                    fill_energy=do_doppler(calib_en, mod, chn);
                    coincidence_gg_GE->Fill(fill_energy);
                    coincidence_gg_ab_GE->Fill(calib_en);

                }
            }
            if (abs(time-ring_time)<40 && (add_back_use_time1+200)>background_gate_lower && (add_back_use_time1+200)<background_gate_upper&&calib_en<10000){
        ...
        if (ab_event_cl3==true && chn==ab_use_crystal_cl3 && mod==3 && ring_sector_time_gate==true && abs(ring_en_for_hist - sector_en_for_hist)<diagonal_cut_width){
            ...
            if (abs(time-ring_time)<40 && (add_back_use_time3+200)>time_gate_lower && (add_back_use_time3+200)<time_gate_upper&&calib_en<10000 ){
                hPixieADC_pg[mod][chn]->Fill(fill_energy);
                hPixieADC_ab[mod][chn]->Fill(ab_use_energy_cl3);
                ring_en_gamma_en->Fill(beta_ring_en, fill_energy);
            }
            if (abs(time-ring_time)<40 && (add_back_use_time3+200)>background_gate_lower && (add_back_use_time3+200)<background_gate_upper&&calib_en<10000){
                time_background[mod][chn]->Fill(fill_energy);
        ...
        if(allready_filled==false && calib_en>20 && ring_sector_time_gate==true && abs(ring_en_for_hist - sector_en_for_hist)<diagonal_cut_width){
            ...
            ////////////////////////////////////////////////////////////checking gg coincidences
            if(mod==2&&(chn==0 || chn==1 || chn==2 || chn==3 || chn==4 || chn==5)&&calib_en<10000 && (time+200)> time_gate_lower_La && (time+200)<time_gate_upper_La && coincidence_gate_labr==true){
                //for the LaBr we want to doppler shift first and then calibrate. For the Ge we want to calibrate, Add-back then doppler shift.
                beta=do_beta(beta_ring_en);
                LaBr_uncalib_ds_gg=do_doppler(calib_en, mod, chn);
                Labr_ds_coinc=do_calibrate(LaBr_uncalib_ds_gg,mod,chn);
                Labr_ab_coinc=do_calibrate(en,mod,chn);

                coincidence_gg_labr->Fill(Labr_ds_coinc);
                coincidence_gg_ab_labr->Fill(Labr_ab_coinc);
                ////////////////////////////////////////////////////////////
                
            }
            ////////////////////////////////////////////////////////////checking gg coincidences
            if(((mod==1&&(chn==0 || chn==1 || chn==2 || chn==3))||(mod==3&&(chn==0 || chn==1 || chn==2 || chn==3 || chn==5)))&& calib_en<10000 && (time+200)>time_gate_lower && (time+200)<time_gate_upper && coincidence_gate_GE==true){
                beta=do_beta(beta_ring_en);
                fill_energy_coinc=do_doppler(calib_en, mod, chn);

                coincidence_gg_GE->Fill(fill_energy_coinc);
                coincidence_gg_ab_GE->Fill(calib_en);
           ...
           /* if ((time+200)>time_gate_si_lower && (time+200)<time_gate_si_upper){
            ...
            if(mod == energy_sharing_ring_mod_ignore && chn == energy_sharing_ring_chn_ignore){
            ...
            else if ((time-ring_time+200)>199&&(time-ring_time+200)<226){
                ...
                if(mod==2&&(chn==0 || chn==1 || chn==2 || chn==3 || chn==4 || chn==5)&&calib_en<10000 && (time+200)> time_gate_lower_La && (time+200)<time_gate_upper_La){
                    ...
                    ////////////////////////////////////////////////////////////checking pgg coincidences
                    if (coincidence_gate_labr==true){
                        coincidence_pgg_labr->Fill(fill_energy);
                        coincidence_pgg_ab_labr->Fill(calib_en);
                    }
                    ////////////////////////////////////////////////////////////
                    //ring_en_sector_en->Fill(ring_en_for_hist, sector_en_for_hist);
                }
                if(((mod==1&&(chn==0 || chn==1 || chn==2 || chn==3))||(mod==3&&(chn==0 || chn==1 || chn==2 || chn==3 || chn==5)))&& calib_en<10000   && (time+200)>time_gate_lower && (time+200)<time_gate_upper){
                    ...
                    ////////////////////////////////////////////////////////////checking pgg coincidences
                    if (coincidence_gate_GE==true){
                        coincidence_pgg_GE->Fill(fill_energy);
                        coincidence_pgg_ab_GE->Fill(calib_en);
                    }
                    ////////////////////////////////////////////////////////////
                    //ring_en_sector_en->Fill(ring_en_for_hist, sector_en_for_hist);
                }
            ...
            else if((time-ring_time+200)>399&&(time-ring_time+200)<1001){ //gamma-particle background
...
Double_t do_calibrate(Double_t raw_energy, Int_t module, Int_t channel){
 ...
 Double_t do_doppler(Double_t en, Int_t gamma_mod, Int_t gamma_chn){
    ...
    /* if (en>5000 && print_army_count<100 && gamma_mod == 2 && (gamma_chn == 4 || gamma_chn==5) ){
     ...
     if (en>5000 && print_oslo_count<100 && gamma_mod == 2 && (gamma_chn == 0 || gamma_chn==1 || gamma_chn==2 || gamma_chn==3) ){
         cout << "Energy before: " << en << " DS energy: " << dop_cor_en << " Alpha: " << cos_alpha <<  " Theta_p " << theta[ring_mod][ring_chn]  << " Phi_p " << phi[sector_mod][sector_chn] << " theta_gamma " << theta[gamma_mod][gamma_chn]<< " phi_gamma " << phi[gamma_mod][gamma_chn] << " Beta " << beta << " Det mod " << gamma_mod << " chn, " << gamma_chn << endl;
         print_oslo_count++;
     }
     if (en>5000 && print_clover_count<100 && (gamma_mod == 1 || gamma_mod == 3)){
         cout << "Energy before: " << en << " DS energy: " << dop_cor_en << " Alpha: " << cos_alpha <<  " Theta_p " << theta[ring_mod][ring_chn]  << " Phi_p " << phi[sector_mod][sector_chn] << " theta_gamma " << theta[gamma_mod][gamma_chn]<< " phi_gamma " << phi[gamma_mod][gamma_chn] << " Beta " << beta << " Det mod " << gamma_mod << " chn, " << gamma_chn << endl;
         print_clover_count++;
     }*/
          return dop_cor_en;
...
void init_histos() {
    ...
    coincidence_pgg_GE = new TH1F("coincidenc_pgg_GE","",10000,0,9999);
    coincidence_gg_GE = new TH1F("coincidenc_gg_GE","",10000,0,9999);
    coincidence_pgg_ab_GE = new TH1F("coincidenc_pgg_ab_GE","",10000,0,9999);
    coincidence_gg_ab_GE = new TH1F("coincidenc_gg_ab_GE","",10000,0,9999);
    coincidence_pgg_labr = new TH1F("coincidenc_pgg_labr","",10000,0,9999);
    coincidence_gg_labr = new TH1F("coincidenc_gg_labr","",10000,0,9999);
    coincidence_pgg_ab_labr = new TH1F("coincidenc_pgg_ab_labr","",10000,0,9999);
    coincidence_gg_ab_labr = new TH1F("coincidenc_gg_ab_labr","",10000,0,9999);
