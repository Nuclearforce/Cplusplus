...
TFile f_hist("../histogram/14C/30000_45500_ex_gate.root", "recreate");
...
Double_t lower_lim = 32000;
Double_t upper_lim = 39000;
...
void three_leaves_run_parrallel()
{
    ...
    for(j=0;j<4;j++){
        for(i=0;i<16;i++){
            hPixieADC_pg[j][i]->Add(time_background[j][i],-(41.0/600.0));
            hPixieADC_ab[j][i]->Add(time_background_not_dopler[j][i],-(41.0/600.0));
            //hPixieADC_eff[j][i]->Add(time_background_eff[j][i],-(41.0/600.0));
    ...
    Clover1->Add(time_background[1][0],-(41.0/600.0));
    Clover1->Add(time_background[1][1],-(41.0/600.0));
    Clover1->Add(time_background[1][2],-(41.0/600.0));
    Clover1->Add(time_background[1][3],-(41.0/600.0));
    Clover3->Add(hPixieADC_pg[3][0],1);
    Clover3->Add(hPixieADC_pg[3][1],1);
    Clover3->Add(hPixieADC_pg[3][2],1);
    Clover3->Add(hPixieADC_pg[3][3],1);
    Clover3->Add(time_background[3][0],-(41.0/600.0));
    Clover3->Add(time_background[3][1],-(41.0/600.0));
    Clover3->Add(time_background[3][2],-(41.0/600.0));
    Clover3->Add(time_background[3][3],-(41.0/600.0));
    ...
    Clover1ab->Add(time_background_not_dopler[1][0],-(41.0/600.0));
    Clover1ab->Add(time_background_not_dopler[1][1],-(41.0/600.0));
    Clover1ab->Add(time_background_not_dopler[1][2],-(41.0/600.0));
    Clover1ab->Add(time_background_not_dopler[1][3],-(41.0/600.0));
    ...
    Clover3ab->Add(time_background_not_dopler[3][0],-(41.0/600.0));
    Clover3ab->Add(time_background_not_dopler[3][1],-(41.0/600.0));
    Clover3ab->Add(time_background_not_dopler[3][2],-(41.0/600.0));
    Clover3ab->Add(time_background_not_dopler[3][3],-(41.0/600.0));
...
void first_loop(){
    ...
    for (entry_id = 0, entry_size = TRenergy.GetSize(); entry_id < entry_size; ++entry_id) {
        ...
        if( (mod==1 && (chn==8||chn==9||chn==10||chn==11||chn==12||chn==13||chn==14||chn==15)) || (mod==2 && (chn==8||chn==9||chn==10||chn==11||chn==12||chn==13||chn==14||chn==15)) || (mod==3 && (chn==7||chn==8||chn==9||chn==10||chn==12||chn==13||chn==14||chn==15)) ) {
            ...
            if(ring_counter>1||calib_en<5000){
                bad_particle=true;
                //cout <<"bad_particle"<<endl;
            }
        ...
        if(mod==0){
            ...
            if(sector_counter>1 || calib_en < 5000){
                bad_particle=true;
...
void second_loop(){
    ...
    Double_t time_gate_upper=240;
    
    Double_t time_gate_si_lower=198;
    Double_t time_gate_si_upper=235;
    ...
    for (entry_id = 0, entry_size = TRenergy.GetSize(); entry_id < entry_size; ++entry_id) {
        ...
        if (energy_sharing_sector==true && chn==energy_sharing_sector_chn_use && mod==0){
            ...
            if (abs(gamma_time-ring_time)<20 && (time+200)>time_gate_lower && (time+200)<time_gate_upper && ring_sector_time_gate==true){
                hPixieADC_ab[mod][chn]->Fill(energy_sharing_sector_en);
                hPixieADC_pg[mod][chn]->Fill(energy_sharing_sector_en);
            }
            if (abs(gamma_time-ring_time)<20 && (time+200)>background_gate_lower && (time+200)<background_gate_upper && ring_sector_time_gate==true){
        ...
        if (ab_event_cl1==true && chn==ab_use_crystal_cl1 && mod==1 && ring_sector_time_gate==true){
            ...
            if (abs(gamma_time-ring_time)<20 && (time+200)>time_gate_lower && (time+200)<time_gate_upper){
            ...
            if (abs(gamma_time-ring_time)<20 && (time+200)>background_gate_lower && (time+200)<background_gate_upper){
        ...
        if(allready_filled==false && calib_en>20 && ring_sector_time_gate==true){
            ...
            if (abs(gamma_time-ring_time)<20 && (time+200)>time_gate_lower && (time+200)<time_gate_upper){
                ...
                else if(mod==2&&(chn==0 || chn==1 || chn==2 || chn==3 || chn==4 || chn==5)&&calib_en<10000 ){
                    //for the LaBr we want to doppler shift first and then calibrate. For the Ge we want to calibrate, Add-back then doppler shift.
                    beta=do_beta(beta_ring_en);
                ...
                else if(((mod==1&&(chn==0 || chn==1 || chn==2 || chn==3))||(mod==3&&(chn==0 || chn==1 || chn==2 || chn==3 || chn==5)))&&calib_en<10000 ){
                    beta=do_beta(beta_ring_en);
                    fill_energy=do_doppler(calib_en, mod, chn);
                    hPixieADC_pg[mod][chn]->Fill(fill_energy);
                    hPixieADC_ab[mod][chn]->Fill(calib_en);
                }
                else{ //i.e. if it is the silicon
...
void make_chain(){
    ...
    DataChain.Add("/Volumes/CBRITS4TB/c0dea36f-31d2-44d7-8418-e1a6319e7468/14C/run-08*.root");
