...
TFile f_hist("../histogram/14C/ab_test.root", "recreate");
...
TH1F* ring_up;
TH1F* ring_down;
TH1F* hPixieADC_no_ab[4][4];
TH1F* hPixieADC_no_ab_cl1;
TH1F* hPixieADC_no_ab_cl3;
...
void three_leaves_14_ab_test()
{
    ...
    Long64_t max_event_testing = 1e6;
    start = std::clock();
    ...
    hPixieADC_no_ab_cl1->Add(hPixieADC_no_ab[1][0],1.0);
    hPixieADC_no_ab_cl1->Add(hPixieADC_no_ab[1][1],1.0);
    hPixieADC_no_ab_cl1->Add(hPixieADC_no_ab[1][2],1.0);
    hPixieADC_no_ab_cl1->Add(hPixieADC_no_ab[1][3],1.0);
    hPixieADC_no_ab_cl3->Add(hPixieADC_no_ab[3][0],1.0);
    hPixieADC_no_ab_cl3->Add(hPixieADC_no_ab[3][1],1.0);
    hPixieADC_no_ab_cl3->Add(hPixieADC_no_ab[3][2],1.0);
    hPixieADC_no_ab_cl3->Add(hPixieADC_no_ab[3][3],1.0);
...
void second_loop(){
    ...
    for (entry_id = 0, entry_size = TRenergy.GetSize(); entry_id < entry_size; ++entry_id) {
        ...
        //////////////////////////no_add_back_histograms///////////////////////////////////////
        
        if((mod==1&&(chn==0||chn==1||chn==2||chn==3))||(mod==3&&(chn==0||chn==1||chn==2||chn==3))){
            if (abs(gamma_time-ring_time)<40 && (time+200)>time_gate_lower && (time+200)<time_gate_upper && calib_en<10000&& calib_en>20){
                hPixieADC_no_ab[mod][chn]->Fill(calib_en);
            }
        }
...
void init_histos() {
    ...
    hPixieADC_no_ab_cl1= new TH1F(Form("ADC_no_ab_cl1"),"",particle_bins,particle_min,particle_max);
    hPixieADC_no_ab_cl3= new TH1F(Form("ADC_no_ab_cl3"),"",particle_bins,particle_min,particle_max);
    ...
    for(Int_t j=0;j<4;j++){
        ...
        for(Int_t i=0;i<4;i++){
            hPixieADC_no_ab[j][i] = new TH1F(Form("ADC_no_ab_test%d_ch%02d",j,i),"",particle_bins,particle_min,particle_max);
        }
    }
