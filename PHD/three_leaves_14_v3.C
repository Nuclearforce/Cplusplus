...
//Now only God knows.
//
//If you are using this code and it breaks please increase
//this counter as a warning for the next programmer:
//
//total_hours_wasted_here:2008
...
TFile f_hist("../histogram/14C/07_10_2018_141.root", "recreate");
...
Double_t lower_lim = 24000;
Double_t upper_lim = 40000;
...
TH2F *ring_en_gamma_en_ab;
TH2F *ring_en_gamma_en_background_ab;
TH1I* rings_fired;
TH1I* sectors_fired;
TH1F* ring_middel;
TH1F* ring_up;
TH1F* ring_down;
TH1F* beta_distribution;
...
//energy_sharing
Int_t energy_sharing_ring_chn_use=0;
Int_t energy_sharing_ring_mod_use=0;
Int_t energy_sharing_ring_chn_ignore=0;
Int_t energy_sharing_ring_mod_ignore=0;
Int_t energy_sharing_sector_chn_use=0;
Int_t energy_sharing_sector_chn_ignore=0;
...
void three_leaves_14_v3()
{
    ...
    while (myReader.Next()) {
        ...
        if(bad_particle==true||bad_gamma==true){
        ...
        else if (gate_on_excitation_spectra==true&&ring_sector_time_gate==true){
            //cout<<"good event" <<endl;
            if(gamma_counter==0){
                cout<<gamma_counter<<" mistake gamma 0" <<endl;
            }
            else if(gamma_counter==1&& (ab_event_cl1==true || ab_event_cl3==true)){
                cout<<gamma_counter<<" asback mistake_1 " <<endl;
            }
            else if(gamma_counter==2 && ab_event_cl1==false && ab_event_cl3==false){
                cout<<gamma_counter<<" asback mistake_1 " <<endl;
            }
            else if(gamma_counter>2){
                cout<<gamma_counter<<" gamma more than 2 no addback " <<endl;
            }
            ...
            if(ring_counter==0){
                cout<<"mistake_0 ring"<<endl;
            }
            else if(ring_counter==1&&ring_en_for_hist==0){
                cout<<"mistake 0 sector"<<endl;
            }
            else if(ring_counter==2&&energy_sharing_ring==false){
                cout<<" mistake_2 ring"<<endl;
            }
            else if(ring_counter>2){
                cout<<"mistake_3 ring"<<endl;
            }
            
            if(sector_counter==0){
                cout<<"mistake 0 sector"<<endl;
            }
            else if(sector_counter==1&&sector_en_for_hist==0){
                cout<<"mistake 0 sector"<<endl;
            }
            else if(sector_counter==2&&energy_sharing_sector==false){
                cout<<" mistake 2 sector"<<endl;
            }
            else if(sector_counter>2){
                cout<<"mistake 3 sector"<<endl;
            }
            
            if(ring_en_for_hist<lower_lim||ring_en_for_hist>upper_lim){
                cout<<"mistake particle gate "<< ring_en_for_hist <<endl;
            }
            second_loop();
            //cout<<"two"<<endl;
        }
    ...
    Int_t j=0;
    Int_t i=0;
    Double_t Labr_subtract=16.0;
    Double_t Ge_subtract=16.0;
    Double_t background_size=100.0;
    for(j=0;j<4;j++){
        for(i=0;i<16;i++){
            if (j==2&&(i==0||i==1||i==2||i==3||i==4||i==5)){
                ...
                hPixieADC_pg[j][i]->Add(time_background_pg[j][i],-0.16);
                hPixieADC_ab[j][i]->Add(time_background_not_dopler_pg[j][i],-0.16);
...
void first_loop(){
    ...
    energy_share_use_time_ring=999999;
    energy_share_use_time_sector=99999;
    gamma_time=99999;
    bad_gamma=false;
    ...
    //coincidence
    Int_t labr_counts=0;
    Int_t clover_counts=0;
    Int_t labr0_counter =0;
    Int_t labr1_counter =0;
    Int_t labr2_counter =0;
    Int_t labr3_counter =0;
    Int_t ge1_counter =0;
    Int_t ge2_counter =0;
    Int_t ge3_counter =0;
    ...
    for (entry_id = 0, entry_size = TRenergy.GetSize(); entry_id < entry_size; ++entry_id) {
        ...
        if( (mod==1 && (chn==8||chn==9||chn==10||chn==11||chn==12||chn==13||chn==14||chn==15)) || (mod==2 && (chn==8||chn==9||chn==10||chn==11||chn==12||chn==13||chn==14||chn==15)) || (mod==3 && (chn==7||chn==8||chn==9||chn==10||chn==11||chn==12||chn==13||chn==14||chn==15)) ) {
            ...
            if (ring_counter==0){
                ...
                //cout<<"one_particle"<<endl;
            }
            else if(ring_counter==1){
                if(abs(energy_sharing_ring_chn_use-chn)==1&&abs(time-energy_share_use_time_ring)<101){
                    energy_sharing_ring=true;
                    ring_en_for_hist=ring_en_for_hist+en;
                    if(time<energy_share_use_time_ring){
                        energy_share_use_time_ring=time;
                        energy_sharing_ring_chn_ignore=energy_sharing_ring_chn_use;
                        energy_sharing_ring_mod_ignore=energy_sharing_ring_mod_use;
                        energy_sharing_ring_chn_use=chn;
                        energy_sharing_ring_mod_use=mod;
                    }
                    else{
                        energy_sharing_ring_chn_ignore=chn;
                        energy_sharing_ring_mod_ignore=mod;
                    }
                    // cout<<"two_particle_good"<<endl;
                }
                else{
                    //cout<<"more than two bad"<<endl;
                    bad_particle=true;
                }
            }
            else{
        ...
        if(mod==0){
            ...
            else if (sector_counter==1){
                if(abs(energy_sharing_sector_chn_use-chn)==1&&abs(time-energy_share_use_time_sector)<101){
                    sector_en_for_hist=sector_en_for_hist+en;
                    energy_sharing_sector=true;
                    if(time<energy_share_use_time_sector){
                        energy_share_use_time_sector=time;
                        energy_sharing_sector_chn_ignore=energy_sharing_sector_chn_use;
                        energy_sharing_sector_chn_use=chn;
                    }
                    else{
                        energy_sharing_sector_chn_ignore=chn;
                    }
                }
                else{
                    //cout<<"more than two bad"<<endl;
        ...
        if (mod==1 && (chn==0 || chn==1 || chn==2 || chn==3) && calib_en < 10000 && calib_en>20){
            ...
            if (abs(time-add_back_use_time1)<101.0 && count_cl1==1 && (abs(chn-ab_oldlocation_cl1)==1||abs(chn-ab_oldlocation_cl1)==3)){
                ab_event_cl1=true;
                ab_use_energy_cl1=ab_olden_cl1+calib_en;
                
                if(time<add_back_use_time1){
                    add_back_use_time1=time;
                    ab_use_crystal_cl1=chn;
                    ab_ignore_crystal_cl1=ab_oldlocation_cl1;
                }
                else{
            ...
            if(count_cl1>2||ab_olden_cl1+calib_en>15000){
                ab_event_cl1=false;
            }
            count_cl1++;
        }
        ...
        if (mod==3 && (chn==0 || chn==1 || chn==2 || chn==3) && calib_en < 10000 && calib_en>20){
            ...
            if (abs(time-add_back_use_time3)<101.0 && count_cl3==1 && (abs(chn-ab_oldlocation_cl3)==1||abs(chn-ab_oldlocation_cl3)==3)){
                ab_event_cl3=true;
                ab_use_energy_cl3=ab_olden_cl3+calib_en;
                if(time<add_back_use_time3){
                    add_back_use_time3=time;
                    ab_use_crystal_cl3=chn;
                    ab_ignore_crystal_cl3=ab_oldlocation_cl3;
                }
                else{
            ...
            if(count_cl3>2||ab_olden_cl3+calib_en>15000){
                ab_event_cl3=false;
            }
            count_cl3++;
        }
        
        /////////////////getting rid of bad gammas///////////////////////
        
        ///////////////only 1 gammma///////////////////////
        if( ((mod==1 && (chn==0 || chn==1 || chn==2 || chn==3)) || (mod==3 && (chn==0 || chn==1 || chn==2 || chn==3||chn==5)) || (mod==2 && (chn==0 || chn==1 || chn==2 || chn==3 || chn==4 || chn==5))) && calib_en < 15000 && calib_en>20){
            if (gamma_counter==0){
                gamma_time=time;
            }
            if (gamma_counter==1){
            ...
            if(abs(gamma_amount_of_detectors_check-mod)>0 && gamma_counter>0){
                bad_gamma=true;
            }
            
            if(mod==2 && (chn==0 || chn==1 || chn==2 || chn==3)){
                LaBr_gamma_chn=chn;
                if (LaBr_counter>0 && abs(LaBr_gamma_chn-chn)>0){
                    bad_gamma=true;
                }
                LaBr_counter++;
            }
            ...
            gamma_counter++;
            if(mod==3&&chn==5){
                gamma_amount_of_detectors_check=8;
            }
            else{
        ...
        if( ((mod==1 && (chn==0 || chn==1 || chn==2 || chn==3)) || (mod==3 && (chn==0 || chn==1 || chn==2 || chn==3||chn==5)) || (mod==2 && (chn==0 || chn==1 || chn==2 || chn==3 || chn==4 || chn==5))) && ((calib_en > 15000) || (calib_en<8))){
            bad_gamma=true;
        }
        ...
        if(mod==1 && (chn==0 || chn==1 || chn==2 || chn==3)){
            ge1_counter++;
        }
        if(mod==3 && (chn==0 || chn==1 || chn==2 || chn==3)){
            ge2_counter++;
        }
        if(mod==3 && chn==5){
            ge3_counter++;
    ...
    labr_counts=labr0_counter+labr1_counter+labr2_counter+labr3_counter+ge3_counter;
    clover_counts=ge1_counter+ge2_counter;
    
    if(labr0_counter>1||labr1_counter>1||labr2_counter>1||labr3_counter>1||ge3_counter>1){
        bad_gamma=true;
    }
    if(gamma_counter==0){
        bad_gamma=true;
    ...
    if(ab_event_cl3==false&&ab_event_cl1==false){
        if(gamma_counter<1||gamma_counter>1){
            bad_gamma=true;
        }
    }
    if(ab_event_cl3==true&&ab_event_cl1==false){
        if(gamma_counter<2||gamma_counter>2){
            bad_gamma=true;
        }
    }
    if(ab_event_cl1==true&&ab_event_cl3==false){
        if(gamma_counter<2||gamma_counter>2){
            bad_gamma=true;
        }
    }
    if(ab_event_cl1==true&&ab_event_cl3==true){
        bad_gamma=true;
    }
    
    /* if(do_beta(ring_en_for_hist)<0.07||do_beta(ring_en_for_hist)>0.085){
     bad_particle=true;
     bad_gamma=true;
     }*/
    ...
    if(ring_en_for_hist>lower_lim && ring_en_for_hist<upper_lim && sector_en_for_hist>lower_lim && sector_en_for_hist<upper_lim){
        gate_on_excitation_spectra=true;
    }
    
    ////////////////////excluding 0 particles/////////////////////////////
    ...
    //////////////ring_sector_time gate//////////////////////
    
    if( (energy_share_use_time_ring-energy_share_use_time_sector+200)>195 && (energy_share_use_time_ring-energy_share_use_time_sector+200)<210){
        ring_sector_time_gate=true;
    }
    /*if(bad_gamma==false&&bad_particle==false&&gate_on_excitation_spectra==true){
...
void second_loop(){
    ...
    Double_t time_gate_upper=204;
    Double_t time_gate_lower_La=198;
    Double_t time_gate_upper_La=204;
    Double_t time_gate_si_lower=198;   //used for efficiency
    Double_t time_gate_si_upper=213;   //used for efficiency
    Double_t diagonal_cut_width=2250;
    Double_t LaBr_uncalib_ds_gg=0;
    Double_t pg_gate_lower = 999;
    Double_t pg_gate_upper =1016;
    Double_t pg_background_lower =1300;
    Double_t pg_background_upper=1350;
    Double_t pg_background_lower_two =300;
    Double_t pg_background_upper_two =350;
    Double_t labr_calib_energy=0;
    ...
    //energy_share_use_time_ring=gamma_time;
    
    Double_t background_gate_lower=250;
    Double_t background_gate_upper=350;
    Double_t fill_energy = 0.0;
    Double_t LaBr_uncalib_ds;
    Bool_t allready_filled;
    
    if (abs(ring_en_for_hist - sector_en_for_hist)<diagonal_cut_width){
        ring_sector_time->Fill(energy_share_use_time_sector-energy_share_use_time_ring+200);
    ...
    for (entry_id = 0, entry_size = TRenergy.GetSize(); entry_id < entry_size; ++entry_id) {
        ...
        if( (mod==1&&(chn==0||chn==1||chn==2||chn==3))||(mod==3&&(chn==0||chn==1||chn==2||chn==3||chn==5)) || ( mod==2&&(chn==0||chn==1||chn==2||chn==3))){
            pg_time->Fill(time-energy_share_use_time_ring+1000);
        }
        ...
        if (mod==3&&chn==13){
            ...
            if(abs(ring_en_for_hist - sector_en_for_hist)<diagonal_cut_width){
                ...
                if (abs(gamma_time-energy_share_use_time_ring)<15){
        ...
        if (energy_sharing_sector==true && chn==energy_sharing_sector_chn_use && mod==0){
            ...
            if ((time-energy_share_use_time_ring+1000)>pg_gate_lower&&(time-energy_share_use_time_ring+1000)<pg_gate_upper &&abs(ring_en_for_hist - sector_en_for_hist)<diagonal_cut_width){
                hPixieADC_ab[mod][chn]->Fill(sector_en_for_hist);
                hPixieADC_pg[mod][chn]->Fill(sector_en_for_hist);
            }
            allready_filled = true;
        }
        if(energy_sharing_sector==true && chn==energy_sharing_sector_chn_ignore && mod==0){
            //do nothing
            allready_filled = true;
        }
        ...
        if (energy_sharing_ring==true && chn==energy_sharing_ring_chn_use && mod==energy_sharing_ring_mod_use){
            ...
            if ((time-energy_share_use_time_ring+1000)>pg_gate_lower&&(time-energy_share_use_time_ring+1000)<pg_gate_upper && abs(ring_en_for_hist - sector_en_for_hist)<diagonal_cut_width){
                hPixieADC_pg[mod][chn]->Fill(ring_en_for_hist);
                hPixieADC_ab[mod][chn]->Fill(ring_en_for_hist);
            }
            allready_filled = true;
        }
        if(energy_sharing_ring==true && chn==energy_sharing_ring_chn_ignore && mod==energy_sharing_ring_mod_ignore){
            allready_filled = true;
        }
        /////////////////////////add_back histogram filling////////////////////////////////////////
        if (ab_event_cl1==true && chn==ab_use_crystal_cl1 && mod==1){
            beta=do_beta(ring_en_for_hist);
            fill_energy=do_doppler(ab_use_energy_cl1, mod, chn);
            if ((time-energy_share_use_time_ring+1000)>pg_gate_lower&&(time-energy_share_use_time_ring+1000)<pg_gate_upper && calib_en<15000&& abs(ring_en_for_hist - sector_en_for_hist)<diagonal_cut_width){
                hPixieADC_pg[mod][chn]->Fill(fill_energy);
                hPixieADC_ab[mod][chn]->Fill(ab_use_energy_cl1);
                //ring_en_gamma_en->Fill(ring_en_for_hist, fill_energy);
            }
        ...
        if(allready_filled==false && calib_en>20 && abs(ring_en_for_hist - sector_en_for_hist)<diagonal_cut_width){
            
            
            /*if((mod == energy_sharing_ring_mod_ignore && chn == energy_sharing_ring_chn_ignore) || (mod ==0 && chn == energy_sharing_sector_chn_ignore)){
             //ignore
             }*/
            
            
            
            if ((time-energy_share_use_time_ring+1000)>pg_gate_lower&&(time-energy_share_use_time_ring+1000)<pg_gate_upper){
                
                if(mod==2&&(chn==0 || chn==1 || chn==2 || chn==3)&&calib_en<15000){
                    ...
                    ring_en_gamma_en->Fill(ring_en_for_hist, fill_energy);
                    ring_en_gamma_en_ab->Fill(ring_en_for_hist, calib_en);
                    hPixieADC_pg[mod][chn]->Fill(fill_energy);
                    hPixieADC_ab[mod][chn]->Fill(calib_en);
                    //ring_en_sector_en->Fill(ring_en_for_hist, sector_en_for_hist);
                }
                else if(((mod==1&&(chn==0 || chn==1 || chn==2 || chn==3))||(mod==3&&(chn==0 || chn==1 || chn==2 || chn==3 || chn==5)))&& calib_en<15000){
                    beta=do_beta(ring_en_for_hist);
                    fill_energy=do_doppler(calib_en, mod, chn);
                    ring_en_gamma_en->Fill(ring_en_for_hist, fill_energy);
                    ring_en_gamma_en_ab->Fill(ring_en_for_hist, calib_en);
                ...
                else if(calib_en>10){ //i.e. if it is the silicon
            ...
            else if(((time-energy_share_use_time_ring+1000)>(pg_background_lower)&&(time-energy_share_use_time_ring+1000)<(pg_background_upper))||((time-energy_share_use_time_ring+1000)>(pg_background_lower_two)&&(time-energy_share_use_time_ring+1000)<(pg_background_upper_two))){ //gamma-particle background
                
                if(mod==2&&(chn==0 || chn==1 || chn==2 || chn==3)&&calib_en<15000){
                    //for the LaBr we want to doppler shift first and then calibrate. For the Ge we want to calibrate, Add-back then doppler shift.
                    beta=do_beta(ring_en_for_hist);
                    fill_energy=do_doppler(calib_en,mod,chn);
                    ring_en_gamma_en_background->Fill(ring_en_for_hist, fill_energy);
                    ring_en_gamma_en_background_ab->Fill(ring_en_for_hist, calib_en);
...
Double_t do_doppler(Double_t en, Int_t gamma_mod, Int_t gamma_chn){
    Double_t dop_cor_en;
    Double_t cos_alpha;
    cos_alpha=sin(theta[energy_sharing_ring_mod_use][energy_sharing_ring_chn_use])*sin(theta[gamma_mod][gamma_chn])*cos(phi[0][energy_sharing_sector_chn_use]-phi[gamma_mod][gamma_chn]) + cos(theta[energy_sharing_ring_mod_use][energy_sharing_ring_chn_use])*cos(theta[gamma_mod][gamma_chn]);
    //cos_alpha=cos(theta[gamma_mod][gamma_chn]-theta[energy_sharing_ring_mod_use][energy_sharing_ring_chn_use]);
    dop_cor_en=en*(1.0-beta*cos_alpha)/(sqrt(1-beta*beta));
    if(gamma_mod==2&&gamma_chn==0){
        beta_distribution->Fill(beta);
    }
    //Int_t count_print = 0;
    /*if (gamma_mod==2&&gamma_chn==1&&count_print<50){
        cout << "cos alpha " << cos_alpha << " energy before " << en << " doppler shifted energy " << dop_cor_en << " theta_p " << theta[energy_sharing_ring_mod_use][energy_sharing_ring_chn_use]  << " phi_p " << phi[0][energy_sharing_sector_chn_use] << " phi_gamma " << phi[gamma_mod][gamma_chn]<< " theta_gamma " << theta[gamma_mod][gamma_chn] << " beta " << beta << endl;
        count_print++;
     }*/
...
void init_histos() {
    ...
    pg_time = new TH1F("time_particle_gamma","",2501,0,2500);
    ...
    ring_en_gamma_en_ab = new TH2F("ring_en_gamma_en_ab","",1000,particle_min,particle_max,1000,particle_min,9000);
    ring_en_gamma_en_background_ab = new TH2F("ring_en_gamma_en_background_ab","",1000,particle_min,particle_max,1000,particle_min,9000);
    ...
    beta_distribution = new TH1F("beta_distribution","",100,0.05,0.09);
